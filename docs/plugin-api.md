...
- `plugins-src/profile/` は登録アカウントの現在のプロフィール（kind 0）を管理 UI に出す。**状態も DB も持たない第一級の同梱プラグイン**で、ページを開くたびに第 14.7 節の取得の口でリレーから最新の 1 件を取る実例であり、第 13.3 節の `image` のブロックを使う唯一の同梱の例でもある。
