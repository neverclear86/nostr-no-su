...
取り直しで届いたイベントも重複排除のウィンドウに id が残っていれば弾かれるので、戻る範囲は保証されない。
取り直しで届いたイベントは、今のところ取り直しを要求していないプラグインにも配られるので、イベント処理関数を冪等に書くという第 4 章の原則は変わらない。
