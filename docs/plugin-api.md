...
`details` はブロックの中にブロックを置けない。`summary` は折りたたみのボタンの文字列、`text` は開いたときに出す整形済みのテキスト（改行はそのまま、長い行は折り返す）である。`id` は 64 桁の 16 進のような識別子を先頭 10 桁と末尾 6 桁に省略し、コピーのボタンを添えて出す。`image` は `variant` が無ければ URL の指す画像を枠の幅と高さ 192px に収めて切らずに出し、`variant` が `icon` なら 64px の丸いアイコン、`banner` なら枠の幅いっぱいの 3:1 の帯（高さは 192px まで）に切り抜いて出す。`url` の scheme が `http` / `https` でないときは、`variant` に関係なく画像を描かず、代替文だけを枠に出す（節の描画は止まらない）。
