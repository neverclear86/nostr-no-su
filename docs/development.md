...
| `docker-image` | 同じコミットから 2 回ビルドして同じイメージになること、実行イメージの中身、healthcheck、remsh の口、同梱プラグインの位置。`linux/amd64`（`ubuntu-latest`）と `linux/arm64`（`ubuntu-24.04-arm`）で 1 回ずつ走る | `Dockerfile`、`docker/`、`docker-compose.yml`、`plugins-src/`、`vendor/`、`gleam.toml`、`manifest.toml` を変えた |
