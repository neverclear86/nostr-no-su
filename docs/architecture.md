...
問い合わせが失敗したときはその項目だけを、リレーの接続状態は「未接続」（`disconnected`）、プラグインは「応答なし」（`unavailable`）として描画し、アカウント・承認待ち・セッション・リレーは一覧の代わりにその理由を出す。ページ全体は失敗させない。
...
リレーが購読を CLOSED で閉じたら、その購読を開いていないものとして扱い、再試行を予約して張り直す。
この待ちは定義を得られないときの待ちとは別に数え、閉じられるたびに倍に延び、張り直しの依頼で定義を得ると初期値に戻る。
...
プラグインのランナーは、起動時（本体の再起動かクラッシュからの復帰）と `Disabled` からの再有効化で、取り直しの要求を立てて監視の購読を張り直させる。
取り直しの購読はプラグインごとに `nostr-no-su-catchup-<プラグイン名>` の id を持ち、`authors` は監視の購読と同じ署名者で、範囲は `since` から `until` までに閉じる。
`since` はランナーのメモリの再開点があればそれを、無ければ DB の `plugin_resume` の値を使い、`until` は要求を立てた時刻である。
それより後のイベントは通常の監視の購読が運ぶ。
保存済みの再開点が無いプラグインは取り直す範囲が決まらないので、購読を定義しない。
取り直しの要求を 1 つでも解けなければ、その評価では購読を 1 本も定義しない。
...
版 5 はプラグインごとの再開点のテーブル（`plugin_resume`）である。ランナーが処理したイベントの `created_at` で前進し、保存のアクターが 5 秒ごとに書く。復帰したランナーの取り直しの購読の `since` にも使う（「監視の購読」の節）。
...
ランナーは起動時と `Disabled` からの再有効化で、再開点からの取り直しを要求して監視の購読を張り直させる（「監視の購読」の節）。
