...
ルートの子を追加する順序には規則があり、規則と逆順で壊れる配線は `src/nostr_no_su/app.gleam` の `start` の行内コメントにある。
...
OTP のどの節がそう振る舞うかの出典は、[設計上の判断と既知の制約](design-decisions.md) の「プラグインが申告した子プロセスは Temporary で載せる」にある。
...
ページとフォームとページ枠（スタイルシート、スクリプト、テーマと言語の切り替え）のパスの定義は `admin/routes.gleam` に置き、ルーティング（`routes.parse` の結果で振り分ける）と、リンク、フォームの `action`、ページ枠の `link` と `script`、承認の URL（`routes.href` で組み立てる）が同じ定義を見る。
...
設定は `PLUGIN_<NAME>_<KEY>` の環境変数を集め、プラグイン名が確定した時点で接頭辞に一致するものだけを切り出し、本体のアカウントストアの接続先を予約キー `DatabaseUrl` で足して渡す。
...
│       ├── admin/routes.gleam    ページとフォームとページ枠のパスの定義（ルートの型、その解析と組み立て、アカウントとリレーへの操作の型）
...
│       ├── admin/view.gleam      ページ枠と、admin/i18n、admin/routes、admin/wordmark 以外の本体のモジュールに依存しない部品（lustre）
...
| 変数 | 読み手 |
| --- | --- |
| `DATABASE_URL`（`_FILE`） | バンカー（アカウントストア）、各プラグイン（予約キー `DatabaseUrl` で渡す） |
