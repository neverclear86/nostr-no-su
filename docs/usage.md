...
![ダッシュボード上部の 5 枚のタイル](images/usage/tiles.png)

上部のタイルは同じページの節へのリンク。承認待ちが 1 件以上あるとタイルが警告色になる。
...
NIP-46 専用のリレーは kind 24133 以外の購読を拒否するので、「監視」のチェックを外す。
...
スマートフォンに渡すときは、アカウントの行の「接続 QR コード」から接続 QR コードのページを開き、カメラかクライアントの読み取り機能で読み取る。
...
| --- | --- | --- |
| 接続 QR コード | 接続 URI と接続 URI（要承認）を QR コードで出す。確認ページは無い | 無し |
