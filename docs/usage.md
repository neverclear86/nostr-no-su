...
アカウントかバンカー用のリレーが 0 件の間は、ダッシュボードの上部に「はじめに」の帯が出て、この 2 つの登録と接続 URI を貼る手順を 3 段で示す。
済んだ段には「済み」が付き、まだの段の「リレーを追加」「アカウントを追加」で登録のダイアログが開く。

### ダッシュボードの見方

![ダッシュボード上部の概要の帯](images/usage/overview.png)
...
承認待ちがあるとき（と一覧を得られないとき）だけ、2 列の上に「承認待ちの接続」の帯が全幅で出る。承認待ちがあるときは 30 秒ごとに読み込み直す。
...
アカウントの節の「アカウントを追加」を開く。

![アカウントの追加](images/usage/new-account.png)

- **既存の秘密鍵を登録**：手元の nsec を貼り付ける。登録するとダッシュボードへ戻る
- **新しい秘密鍵を生成**：サーバーに鍵を作らせる。開いたダイアログの nsec をバックアップしてから「この鍵を登録する」を押す

登録後に nsec を見るには、管理パスワードの再入力が要る（[日々の操作](#日々の操作)）。

### 3. 接続 URI をコピーする

アカウントの行の「接続 URI と操作」を開くと、3 つのコピー欄が出る。
...
スマートフォンに渡すときは、アカウントの行の「接続 QR コード」でダイアログを開き、タブで渡す URI を選ぶ。出ているコードは端末のカメラ用で、読み取ったテキストをコピーし、クライアントの入力欄に `bunker://` と打ってからその後ろに貼る。クライアント自身の読み取り機能を使うときは、コードの下の畳みを開いてその中のコードを読み取る。
...
| `nostrconnect://` | クライアントが出した URI を、セッションの節の「クライアントを接続」に貼り、署名するアカウントを選び、確認のダイアログで URI のリレーと権限を確かめて接続する。URI のリレーは登録しない。バンカーの用途に無いリレーは、それを使うセッションの接続だけに使う | クライアントが URI を出す側のとき |
...
アカウントの行の 5 つのボタン。どれもダイアログを開き、「接続 QR コード」以外はその中のフォームから実行する。

| 操作 | 何が起きるか | クライアントへの影響 |
| --- | --- | --- |
| 接続 QR コード | 接続 URI と接続 URI（要承認）を、カメラ用とクライアントの読み取り機能用の 2 通りの QR コードで出す。ダイアログで開く | 無し |
...
表示した後は「閉じる」を押す。ダッシュボードへ戻り、再読み込みで POST を再送しない。
...
![上部バーのロゴとテーマ・言語の切り替え](images/usage/navbar.png)
...
起動時に読み込めなかったプラグインがあると、概要のプラグインの補足にエラーの色で「読み込み失敗 N」が出て、プラグインの節の末尾に「読み込めなかったプラグイン」の枠がエラーの色で出る。
枠には候補のモジュール名かディレクトリー名と理由が英語のまま並ぶ（ログの `[plugin_loader]` の行と同じ文）。
...
| 「読み込めなかったプラグイン」が出る。概要に「読み込み失敗 N」 | 起動時にそのプラグインを読み込めなかった。枠の理由（ログの `[plugin_loader]` の行）を直して、`.env` なら `docker compose up -d`、`./plugins` の中身なら `docker compose restart nostr-no-su` |

![アカウントの節の末尾の、読み込めなかったアカウントの枠](images/usage/unreadable.png)
