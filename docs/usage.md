...
| `nostrconnect://` | クライアントが出した URI を、セッションの節の「クライアントを接続」に貼り、署名するアカウントを選び、確認の画面で URI のリレーと権限を確かめて接続する。URI のリレーは登録しない。バンカーの用途に無いリレーは、それを使うセッションの接続だけに使う | クライアントが URI を出す側のとき |
