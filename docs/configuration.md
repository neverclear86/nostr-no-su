...
この文書は、環境変数の一覧と、`.env` の書き方、秘密をファイルで渡す方法、管理 UI の待ち受けと認証、リバースプロキシーの置き方、同梱の docker compose の構成の詳細をまとめる。導入の手順は [README（日本語）](../README.ja.md) の「はじめる」、バックアップと更新は [運用](operations.md) にある。
