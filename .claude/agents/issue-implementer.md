...
- コードのコメント（Doc コメントを含む）は `.claude/rules/code-comments.md` を読んで従う。プランが文言を指定していればそのまま使う（`<土台の値 + 1>` の形の件数は、今の土台の値から計算した数で埋める）
...
- Doc を縮めるときは、条件を述べる文の限定語（数、「未訳の」、型の欄の範囲）を残す
- Doc の呼び出し元の列挙を役割の記述に置き換えるときは、先に全呼び出し元を `git grep` し、全部を覆う役割にする
- 消す文や関数が述べる事実と主張は、識別子だけでなく主張の語でも掃き出し、1 つずつコードで確かめて残すか移すかを決める。参照に置き換えるときは、参照先の節にその内容があることを確かめる
...
- PR 本文を貼り直すときは、検査の結果を最新の head のものに置き換えて前の head の段落を消し、「プラン:」のリンクを最新の版に付け替え、「満たす変更」の `path:行` は置き換えた式を `grep -n` で引いて書く（条件への対応と rebase の後も同じ）
...
## rebase を頼まれたら
マージ担当が、PR が main と衝突している、または main とマージした結果が `gleam build --warnings-as-errors` を通らないと判断したときに頼まれる。
作業ツリーで `git fetch origin main && git rebase origin/main` を行い、衝突を解いて `gleam build --warnings-as-errors` と `gleam test` を通し、`git -C <作業ツリー> push --force-with-lease` する。rebase 以外の変更は入れない。ただし衝突が無くても build が落ちるとき（兄弟のマージで型や import が変わったとき）は、main に合わせる最小の直しだけを rebase のコミットに含め、その箇所を reason に書く（マージ担当はこれを衝突の解消として扱い、超える差分は最終確認に再確認させる）。衝突の解き方に設計の判断が要るときは push せず、status を blocked にして reason に理由を書く。成功したら status を rebased にして新しい head を返す。
`git -C <作業ツリー> push --force-with-lease` は、他のコマンドと連結せず 1 回の Bash 呼び出しに 1 つだけ置く。許可 `Bash(git -C * push --force-with-lease*)`（`.claude/settings.json`）は呼び出しの全部の部分コマンドが許可に一致するときだけ効き、連結した呼び出しは auto モードの分類器（Git Destructive）に回って拒否されることがある。
