...
- UI を変える issue（`ui: true`）でだけ、`dev/screenshots.mjs` で main と作業ブランチの両方の画面を撮り（幅 1280 と 375、ライトとダーク。同じ初期状態を作ってから）、PR を作った直後に `gh pr comment <PR> --attach <png>` で「変更前」「変更後」を貼る。言語は日本語（`ja-JP`）を基本とし、英語は別に一式を撮らず、`i18n.gleam` の英語の文言を変えたときにその画面だけを翻訳の確認として貼る。変えた画面と状態（空、エラー、承認待ちなど）を漏らさず、見た目が変わらないときも貼る。UI を変えない issue では撮らない
