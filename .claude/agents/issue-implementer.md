...
3. devin 用の clone を作る: `git clone -q --shared /home/lina/workspace/projects/nostr-no-su <スクラッチパッド>/devin-<N> && git -C <スクラッチパッド>/devin-<N> checkout -q --detach $(git -C <作業ツリー> rev-parse HEAD)`（作業ツリーと同じコミットにする。土台の SHA ではない。並列のマージで origin/main が進んでいると差分が当たらない。jail は /tmp の下の独立 clone だけ受け付ける。worktree は使えない）。clone がすでにあって `DEVIN_REPORT.md` があれば（前の実行が待ちきれずに打ち切られた跡）、devin を起動し直さず手順 6 からその報告を使う。clone があって報告が無く `<clone>.exit` も無ければ、clone を消して作り直す
