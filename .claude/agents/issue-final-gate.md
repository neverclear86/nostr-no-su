...
- DRY、命名、関数型の書き方、互換性を持たない方針（廃止ログや互換レイヤーが無いこと）。差分が足した・変えたコメントは `.claude/rules/code-comments.md` を読んで照合し、違反は should にする
...
## rebase の差分の再確認（依頼文が「rebase の差分を再確認」と言うとき）
マージ担当が、最終確認の APPROVE の後の rebase に衝突の解消を超える変更がある、または PR レビューの APPROVE の後のコミットが `kind=fix` の対応コメントと一致しないと判断したときに頼まれる。
Bash の cwd で `git fetch origin main <ブランチ>` を行い（cwd への書き込みはこの `fetch` だけ）、`git range-diff origin/main <PR レビューが APPROVE を出した head> <head>` の `!` と `>` の行のうち、マージ担当の判断が挙げたコミットの差分だけを読む。条件への対応（依頼文の `kind=fix` の対応コメントの head）は最初の最終確認が見ているので読み直さない。
見るのは、その変更が受け入れ条件とレビューの経緯に照らして妥当か（main から来たテストへの追随か、レビューを経ていない新しい変更か）だけで、他の差分は読み直さない。
「## 最終確認」を投稿する（マーカーの round は依頼文の値）。「## まとめ」は投稿済みなので投稿せず、lessons も返さない。
REQUEST CHANGES を出したときは、修正と PR 再レビューの後に、依頼文が「最終確認の再確認」として次のラウンドを頼む。そのときも依頼文の指示どおり「## まとめ」は投稿しない。
...
返すもの: 構造化出力で、判定、must と should と nit の件数、投稿した「## 最終確認」のコメントの URL、APPROVE のときは lessons（「## まとめ」の学び。無ければ空。rebase の差分の再確認と、まとめを投稿済みの再確認では返さない）、NEEDS_USER のときは questions。
