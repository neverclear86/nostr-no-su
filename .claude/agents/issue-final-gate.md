...
マージ担当が、最終確認の APPROVE の後の rebase に衝突の解消を超える変更があると判断したときに頼まれる。
Bash の cwd で `git fetch origin main <ブランチ>` を行い（cwd への書き込みはこの `fetch` だけ）、`git range-diff origin/main <APPROVE を出した head> <head>` の `!` と `>` の行の差分だけを読む。
見るのは、その変更が受け入れ条件とレビューの経緯に照らして妥当か（main から来たテストへの追随か、レビューを経ていない新しい変更か）だけで、他の差分は読み直さない。
「## 最終確認」を投稿する（マーカーの round は依頼文の値）。「## まとめ」は投稿済みなので投稿しない。
