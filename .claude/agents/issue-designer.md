...
- 画面を見るときは headless で行う（スクラッチパッドに置いた HTML か既存の画像を、playwright-core の `chromium.launch({ headless: true })` のスクリプトで開く）。撮影用のサーバー（`gleam run -m admin_preview`）は build を伴うのでユーザーの作業ツリーでは起動せず、`dev/screenshots.mjs` も使わない。user スコープの Playwright MCP（`mcp__playwright__*`）は headed でユーザーの画面にブラウザーの窓を開き、作業ツリーに `.playwright-mcp/` を残すので使わない。使ったときは返す前に `browser_close` を呼ぶ
