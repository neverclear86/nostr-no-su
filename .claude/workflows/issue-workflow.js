...
//               分割で生まれたサブ issue はスクリプトが足す（designUrl を親から継ぎ、depth 1、parent、tier は親の判定が決めた none か light。再分割はしない）。
//               別の実行で子を回し直すときは、同じ depth / parent / tier / designUrl を issues に直接書く（スキル issue-workflow の「結果の処理」）
