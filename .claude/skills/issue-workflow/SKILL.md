...
- `after` に分割の親を書かない。親は子が全部マージされた時点で満たされるが、残った子があるとその子の番号と状態で `blocked` になるので、依存する子の番号に展開して書く（親の「## 分割の設計」の表の `after` を見る）
- 分割の子を回し直すときは、その子を `issues` に直接書き、`branch` は前の実行の `<親の branch>-<子の番号>` に合わせる（既存のブランチと PR を続きから使う）
