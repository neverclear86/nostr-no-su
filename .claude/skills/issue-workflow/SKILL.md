...
- 分割の子を回し直すときは、その子を `issues` に直接書き、スクリプトの `runSplit` が付ける形に合わせる。`branch` は前の実行の `<親の branch>-<子の番号>`（既存のブランチと PR を続きから使う）、`depth: 1`、`parent: <親の番号>`、`tier` は親の「## 分割の設計」の表の値（無ければ `light`）、親が `ui: true` なら `designUrl` に親の「## デザインの方針」のコメントの URL。`depth` が無い子は判定から入って再分割されうる。`tier` だけ付けても `depth` が無いと、プランが分割を求めたとき `blocked` にならず分割が実行される。`designUrl` が無い `ui: true` の子はデザインの段階が子の分だけもう一度走る
