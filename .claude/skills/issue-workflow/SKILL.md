...
| マージ | `issue-merger` | opus / low | 承認・CI・衝突と、main とマージした結果の `gleam build --warnings-as-errors`（使い捨ての作業ツリー）を確かめて `gh pr merge --squash --delete-branch`。1 件ずつ |
...
- **同時に進める issue は `window` 件**（既定 4）。1 issue につき動くエージェントは常に 1 体なので、同時のエージェント数も `window` になる。マージは 1 件ずつ直列で、衝突と、main とマージした結果のビルドの失敗（`mergeable` は字面の衝突しか見ず、09-26 の #871 と #873 は `MERGEABLE` のまま main を壊した）は実装エージェントの rebase で解く。その `git push --force-with-lease` は `.claude/settings.json` の許可 `Bash(git -C * push --force-with-lease*)` で通す（`gh issue close` と同じく、単独の Bash 呼び出しにする）
- **依存する issue** は `after` に書く。判定・デザイン・プランは、依存先のプランが承認された時点で、その承認済みプランの URL を依頼文に添えて始まる（依存先がプラン無しの tier none や分割なら、その段階を終えた時点で始まる）。実装は依存先のマージを待ち、最後にマージされた依存先のコミットを土台にする。待つ間は `window` の枠を使わない。依存先がプランの前に止まるか、マージされずに終わると `blocked` になり（分割された依存先は子が全部マージされた時点で満たされ、残った子があれば `questions` にその番号と状態が出る）、`after` が循環していれば待たずに `blocked` になる。依存先の実装がプランの前提と食い違えば、実装エージェントの `deviation` からプランの版を上げる。プランが土台に無い兄弟の部品を前提にして構造化出力の `after` を返したときは、スクリプトが承認の後にその番号を依存先に足し、実装だけがそのマージを待つ（この実行に無い番号、循環、依存先がプランの前に止まったときは同じく `blocked`。09-23 の #673 は `after` 無しで #672 の部品を前提にしたプランが承認され、#672 がプランで止まったまま実装に入って `blocked` になった）
...
- **まとめ**：最終確認が APPROVE を出すと、同じエージェントが続けて「## まとめ」を 1 本投稿する（tier、プランと PR レビューのラウンド数、条件の件数、実装起因の must の件数、学び 0〜3 件）。学びは `lessons` で返り、`results` と最後の `log` に集計が出る。次の改修の材料なので、実行の後に `retrospective` が拾う。ラウンド数はその実行で数えた分で、`planUrl` で引き継いだ issue の表には前の実行のプランレビューを含まない旨の断りが付く（`results[].planInherited`）。マージ担当が rebase の差分にレビューが要ると判断したとき（`needsReview`。衝突の解消を超える rebase の変更、または `kind=fix` と一致しない PR レビューの後のコミット）の再確認は、まとめを投稿せず lessons も返さない。再確認が REQUEST CHANGES なら修正と PR 再レビューを経てもう一度最終確認し、その APPROVE もまとめを投稿しない。再確認の後のマージは 1 回だけで、2 回目も要ると判断されたら `stalled`
...
- **実装とレビューの基準**：DRY、シンプルさ、命名、仕様（issue とプラン）への準拠を厳しめに見る。関数型の書き方を重視する。コードのコメントは `.claude/rules/code-comments.md` に従う
...
- **window**：同時に進める件数。既定 4。依存先を待つ issue は枠を使わないので、`after` の連鎖があっても window を下げない（分割の子が多い実行は 8 まで。09-26 の #700 の実行は 8 で stalled 0）。文書を動かす issue は 1。枠切れは Claude Code の一時停止に任せる（対話セッションから起動したときだけ効く。リセットが 24 時間以内のときだけで、週の枠は解けない）
...
- **stalled のプランの続き**：前の実行でプランレビューの往復の上限（2 ラウンド）で `stalled` になった issue は、前の実行の最後の版のファイル（`<前の scratchpad>/plans/{{N}}-v{{V}}.md`）を `prevPlan` に、そのレビュー（`{{N}}-r2.md`）を `prevReview` に書く（両方。同じ scratchpad で回すと新しいラウンドのレビューが `{{N}}-r2.md` を上書きするので、`plans/` の外に写してから渡す）。スクリプトは判定と版 1 を飛ばし、`planNext` の依頼文で版 V+1 から往復を始める（ラウンドはこの実行で 1 から数え、ラウンド 1 のレビューは前の実行のレビューとの照合になる）。UI を変える issue は前の実行の「## デザインの方針」の URL を `designUrl` に書く（無いとデザインの段階がもう一度走る）
...
- `stalled`：往復が収束しなかった issue。`reason` を添えてユーザーに報告し、指示を待つ（プランの論点が割れたなら `decisions` で決めて新しい実行に載せる、実装が難しいなら issue を分ける）。プランレビューの往復の上限で止まったもの（stage `plan`）は、最後の版とレビューを `prevPlan` と `prevReview` に書いて回し直し、続きの版から往復させる（段階 0 の「stalled のプランの続き」。09-23 の #672 はこの形で 2 ラウンドで APPROVE になった）。`reason` が `noMerge:` で始まるものは失敗ではなく、ユーザーに PR のマージを頼む（段階 0 の「マージをユーザーに残す issue」）
...
`blocked` の連鎖（依存先が止まって `stage: deps` になった issue の列）を移すときの `after` は次のとおりに組む。
- 実行は依存の状態をメモリに持ち、別の実行でマージされた依存先を知らない。`after` に書けるのは同じ実行の `issues` にある番号だけで、依存先がすでにマージ済み（別の実行を含む）なら `after` から外す
- `after` に分割の親を書かない。親を `issues` に入れないと「この実行に含まれていない」で `blocked` になり、入れると判定からやり直して分割し直すので、マージされていない子の番号に展開して書く。親に依存していた issue の `after` はマージされていない子の全部にする。子自身の `after` は、親の「## 分割の設計」の表の `after`（空なら親の `after`）からマージ済みを除いたものにする（表の `after` は兄弟どうしの依存で、親に依存する issue が待つ子を示す列ではない）
- 分割の子を回し直すときは、その子を `issues` に直接書き、スクリプトの `runSplit` が付ける形に合わせる。`branch` は前の実行の `<親の branch>-<子の番号>`（既存のブランチと PR を続きから使う）、`depth: 1`、`parent: <親の番号>`、`tier` は親の「## 分割の設計」の表の値（無ければ `light`）、`ui` は親と同じ値、`note` は `#<親の番号> を分割したサブ issue。親の issue のコメント「## 分割の設計」に全体の方針と兄弟との分担がある`、親が `ui: true` なら `designUrl` に親の「## デザインの方針」のコメントの URL。`depth` が無い子は判定から入って再分割されうる。`ui` が無い子は、実装の依頼文からスクリーンショットの指示が消え、PR レビューの依頼文が「UI を変えない issue」になり、`implementer: "devin"` の実行では devin に振られる。`note` が無い子（`planUrl` の無い、`stage: deps` で止まった子）は、プランの依頼文から親の「## 分割の設計」への案内が消える。`tier` だけ付けても `depth` が無いと、プランが分割を求めたとき `blocked` にならず分割が実行される。`designUrl` が無い `ui: true` の子はデザインの段階が子の分だけもう一度走る
...
`args.dryRun` に issue 番号ごとのシナリオを渡すと、エージェントを立てずに制御の流れだけを確かめられる。シナリオは `happy`、`tier-none`（判定が none → プラン無しで実装 → PR レビュー APPROVE → 最終確認 → マージ）、`tier-none-design-must`（none で PR ラウンド 1 が `designMust` → その場でプラン v1 / r1 → 修正 → ラウンド 2 で APPROVE）、`tier-none-deviation`（none の実装が見込みを超えて `deviation` → その場でプラン v1 / r1 → 続きを実装 → マージ）、`pr-conditions`（PR ラウンド 1 が APPROVE ＋ 条件 2 件 → 条件対応 → 再レビュー無しで最終確認）、`approve-with-conditions`（プランレビューがラウンド 1 で条件 2 件つきの APPROVE。実装の依頼文に条件が入る）、`plan2`（プラン 2 ラウンド）、`plan-stall`（2 ラウンドで `stalled`）、`prev-plan`（`issues[0]` に `prevPlan`（`…/{{N}}-v2.md` の形）と `prevReview` を付ける。判定と版 1 を飛ばして版 3 から始まり、ラウンド 1 で REQUEST CHANGES、ラウンド 2 で APPROVE）、`question`、`needs-user`（プランレビュアーが判断を求める）、`pr-needs-user`、`gate-needs-user`、`null`（実装が結果を返さない）、`status-only`（実装が status だけを返し、スクリプトが `gh pr list` で PR を引いて補ってからレビューに進む）、`null-fix`（修正が結果を返さない）、`impl-blocked`、`fix-blocked`、`deviation`、`planurl-deviation`（`planUrl` と組み合わせる）、`replan-reject`（版上げが承認されない）、`replan-question`、`pr2`、`design-must`、`gate`（最終確認で差し戻し）、`split`（判定で 2 件に分割、2 番目は 1 番目の後）、`split-parallel`（判定で依存の無い 2 件に分割）、`split-tier`（判定で 2 件に分割し、1 番目は tier none でプラン無し、2 番目は 1 番目の後の light）、`triage-question`（判定で質問）、`plan-split`（判定は plan だったがプランの調査で分割）、`child-split`（サブ issue の番号に付ける。サブ issue のプランが再分割を求めて `blocked`）、`plan-after`（`after` 無しの 2 件で 2 番目に付ける。プランが 1 番目の部品を前提にして `after` を返し、実装が 1 番目のマージを待つ）、`devin`（`implementer: "devin"` と組み合わせる。実装が `implementedBy: devin` を返し、集計に出る）、`ci-fail`（CI が通らず blocked）、`conflict`（マージで rebase）、`not-ready`（マージの条件を 1 回だけ確かめ直す）、`not-ready-twice`、`not-ready-review`（マージで衝突 → rebase → 確かめ直しでマージ担当が rebase の差分にレビューが要ると判断 → 最終確認が再確認 → もう一度マージ）、`not-ready-review-conflict`（再確認の後のマージでもう一度衝突 → rebase → マージ。依頼文の range の起点が再確認の見た head で、終点が再確認の後の rebase の head）、`not-ready-review-reject`（再確認が REQUEST CHANGES → 修正 → PR 再レビュー → まとめを投稿しない最終確認 → 通常の APPROVE としてマージ）、`not-ready-fix`（PR レビューの条件対応の後、マージ担当が対応のコミットを `kind=fix` と一致しないと判断 → PR レビューの head からの再確認 → マージ）。
