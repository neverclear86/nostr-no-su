...
- `split`：親が分割された。`subIssues`（番号の配列）と `children` の各結果を、それぞれ上の分類で扱う。`children` が全部 `merged` なら「親 #N のサブ issue が全部閉じた」と報告に載せる。親はユーザーが確認してから閉じるので、閉じない
