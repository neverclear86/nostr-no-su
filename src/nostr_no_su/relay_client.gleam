...
/// 依存するフィルタ（`since` など）が常に最新に保たれる。
pub type Subscriptions =
  fn() -> List(#(String, Filter))

/// ハンドシェイクに許す時間。`start` は呼び出し元を最大でこの時間（さらに
/// stratus が上乗せする 100ms）ブロックする。呼び出し元は監視下のアクターであり、
/// ブロック中はスーパーバイザーの停止要求に応答できないため、この値はワーカーの
/// 停止タイムアウト 5000ms（`relay_connection.supervised` を参照）より小さく
/// しておく必要がある。さもないと、応答しないリレーを待っている接続の停止が
/// 強制 kill で終わる。
