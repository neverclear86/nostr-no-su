...
/// クライアントには保護が効かない。ずれが D 秒なら、リレーが再配送しうる `D + 60`
/// 秒のうち `D` 秒ぶんの再起動では起点を越えられず、再実行が起きる（上限は受付
/// ウィンドウの `window_seconds`）。逆に時計が遅れているクライアントのリクエストは、
/// アクターの起動直後、そのずれの秒数ぶんだけ弾かれうる。
pub type Inputs {
