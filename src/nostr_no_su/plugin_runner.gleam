...
////   ディスパッチャーに一切載らない。
//// - **`handle_event/1` はイベント 1 件ごとの使い捨てプロセスで動く。** ランナー
...
/// `handle_event/1` を 1 件動かした結果。`detail` はスタックトレースで、ログに
...
/// `handle_event/1` を使い捨てのプロセスで 1 件動かし、結果を待つ。プラグインの
...
/// `handle_event/1` を監視付きの使い捨てプロセスで動かす。生成と監視は不可分で
