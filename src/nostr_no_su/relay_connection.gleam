...
/// 接続 1 本に必要なものすべて。状態を問い合わせるためのプロセス名、リレーの
/// URL（ログ行の接頭辞に使う）、ソケットの開き方、新しいソケットごとに行う処理、
/// ソケットを失ったとき（再接続を待つ間、および親（スーパーバイザー）からの停止）
/// に行う処理、再接続の待ち時間の延ばし方、待ち時間を初期値に戻すのに要る接続の
/// 継続時間（ミリ秒）。
pub type Settings {
  Settings(
    name: Name(Msg),
