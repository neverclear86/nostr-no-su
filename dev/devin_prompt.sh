...
4. \`examples/\` を変えたら \`erlc -Wall -Werror -o "\$(mktemp -d)" examples/plugins/*/src/*.erl\`。\`vendor/\` を変えたら \`sh dev/check_vendor_stratus.sh\`。\`docker-compose.yml\`、\`docker-compose.release.yml\`、\`.env.example\` のどれかを変えたら \`sh dev/check_env_example.sh\` と \`sh dev/check_release_compose.sh\`。\`plugins-src/\`、\`gleam.toml\`、\`manifest.toml\` を変えたら \`sh dev/check_shared_versions.sh\` と、\`plugins-src/event_logger\` と \`plugins-src/profile\` のそれぞれで \`gleam build --warnings-as-errors\`、\`gleam test\`（\`event_logger\` は同じ Postgres を使う。\`profile\` は DB を使わない）、\`gleam format --check src test\`
