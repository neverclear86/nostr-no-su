...
- README、docs/architecture.md、.env.example など、変更に関係する文書も同じ変更で直す
...
4. \`examples/\` を変えたら \`erlc -Wall -Werror -o "\$(mktemp -d)" examples/plugins/*/src/*.erl\`。\`vendor/\` を変えたら \`sh dev/check_vendor_stratus.sh\`。\`docker-compose.yml\` か \`.env.example\` を変えたら \`sh dev/check_env_example.sh\`。\`plugins-src/\`、\`gleam.toml\`、\`manifest.toml\` を変えたら \`sh dev/check_shared_versions.sh\` と、\`plugins-src/event_logger\` で \`gleam build --warnings-as-errors\`、\`gleam test\`（同じ Postgres を使う）、\`gleam format --check src test\`
...
7. 意味が変わった語（識別子、環境変数、kind、表、画面の数）ごとに \`sh dev/sweep_refs.sh . <語>...\` を回し、README、docs/、.env.example に古い記述が残っていないことを確かめる（0 件でも報告ファイルに語を書く）
