# 本体と plugins-src/event_logger の manifest.toml の packages に共通する名前の版が、
# すべて一致するかを検査する。
# 影に入る側は本体の版で実行されるため、版がずれると読み込み時には何も起きず、実行時に
# undef で壊れる。CI の唯一の自動検出点である。
# プラグインにだけあるパッケージは検査しない。開発用の依存も共通なら検査する。
# 一致すれば件数を出して 0 で、ずれていればずれたパッケージと両方の版を出して 1 で終わる。
...
awk -F '"' '
  ...
  $2 in host {
    ...
    if (host[$2] != $4) { print "version mismatch: " $2 " (host " host[$2] ", plugin " $4 ")"; bad = 1 }
  }
  END {
    if (!stdlib) { print "gleam_stdlib is not found in both manifest.toml files"; exit 1 }
    if (bad) exit 1
    print "the " shared " shared packages match the host versions"
  }' "$root/manifest.toml" "$root/plugins-src/event_logger/manifest.toml" >&2
