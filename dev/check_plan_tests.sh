...
# 1 列目だけで、そこにある `名前_test` のバッククォート内の語をテスト名とする。同じ節の文や
# 「検証の手順」に出る名前は読まない（プランは 1 行に 1 つのテスト名を 1 列目に置く決まり）。
# 実装側は作業ツリーの test/ と plugins-src/*/test/ の `pub fn 名前_test()` から取る。
#
# 結果は Markdown の表（プランのテスト名 / 実装）で出す。プランの名前がすべて実装にあれば
# 0 で、1 件でも無ければ「無し」の行を出して 1 で終わる。「### テスト」の節が無いとき、
# 表の 1 列目にテスト名が 1 つも無いときも、契約の形でないので 1 で終わる。改名したテストは
...
# 表の行の 1 列目から `名前_test` を取る（見出し行と区切り行にはバッククォートが無い）。
...
' "$file" | sort -u)
[ -n "$planned" ] || { echo "「### テスト」の表の 1 列目にテスト名（\`名前_test\`）が無い" >&2; exit 1; }
