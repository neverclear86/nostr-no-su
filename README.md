...
- **バンカーの購読は起動時刻より前に遡らない**: 切断していた間に届いたリクエストを取りこぼさないため、バンカーの購読は `since = 現在時刻 - 60 秒` で張るが、この起点はプロセスの起動時刻を下回らない（`config.bunker_since`）。リプレイ防止の `seen` ウィンドウはプロセス内にしかなく再起動で空になるため、起動より前にリレーへ保存された kind 24133（NIP-01 では保存しない想定だが strfry などは保存する）を拾うと、処理済みのリクエストを新規として再実行してしまう。同一プロセス内の再接続では `seen` が効くので、60 秒の猶予は従来どおり働く。監視側の購読には同じ制限を掛けない。再起動後にストアドイベントを拾い直すのは取りこぼしの回収になり、保存も id が重複する行を無視する冪等な挿入であるため
