...
- **スキーマの版は前向きにだけ自動で進める**: 本体（`schema_version`）と `event_logger`（`event_logger_schema_version`）は、テーブルの定義を版つきの移行として持ち、読み込みのたびに記録された版より新しい移行を順に適用して、移行ごとに版を 1 行記録する。戻す移行は無い。記録された版がビルドより新しい DB に古いビルドを向けると、本体は理由を 1 行出して終了コード 1 で終了し、`event_logger` は保存アクターを止める（再試行しても変わらないため、DB の障害をプロセスの死にしない上の方針の例外である）。古いビルドに戻すには、移行の前に取った DB のバックアップから手作業で戻す必要がある
