...
- **管理 UI**: `http://127.0.0.1:8080/` でアカウントとその `bunker://` 接続 URI、リレーの接続状態、承認待ちの接続要求（承認・拒否）、承認済みセッション（取り消し可）、有効なプラグインとその状態（無効なら再有効化可）を確認できる。アカウントの登録（nsec の入力とサーバー側での鍵の生成）、削除、接続 secret のローテーション、ラベルの編集、管理パスワードの再入力による秘密鍵の再表示、リレーの追加もここで行う。HTTP Basic 認証（ユーザー名 `admin`）で、既定はループバックのみで待ち受ける
...
起動すると `http://127.0.0.1:8080/` で管理 UI にアクセスできる。ダッシュボードで、承認待ちの接続要求の承認と拒否、アカウントの登録と操作、承認済みのセッションの取り消し、リレーの追加を行い、リレーとプラグインの状態を確かめる。画面の構成、操作ごとの結果と状態コード、接続の承認（auth_url フロー）、CSRF の防ぎ方は [管理 UI](docs/admin-ui.md) にある。
