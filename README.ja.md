...
Nostr-no-Su - Nostr の巣 - は、秘密鍵をクライアントに渡さず、`bunker://` で接続したクライアント（nsec.app、noStrudel など）の署名要求にサーバーが応える。
あわせて、登録したアカウントのイベントをリレーから集めてプラグインで処理する。
docker compose で Postgres ごと立ち上がり、鍵とリレーは管理 UI で扱う。

![管理 UI のダッシュボード](docs/images/usage/dashboard-ja.png)

## ✨ 特徴

- **NIP-46 バンカー**：複数アカウントの鍵を 1 台で管理。複数リレー対応、承認フロー（auth_url）つき
- **鍵は暗号化して保存**：AES-256-GCM + マスターキー。Postgres に保存
- **権限の管理**：クライアントごとに署名できる kind と NIP-44 の暗号化を管理 UI で編集
- **自前の暗号実装**：BIP-340 / NIP-44 v2。NIF 不要
- **イベント監視とプラグイン**：自分のイベントを複数リレーから集め、検証と重複排除をしてプラグインへ。BEAM のモジュールを置くだけで追加
- **管理 UI**：アカウント、接続の承認、セッション、リレー、プラグインを 1 画面で。日英、ライト / ダーク、外部ファイルなし
- **止まらない**：OTP のスーパービジョンツリー。リレーは個別に再接続、DB が落ちても再試行
...
リレーとアカウントの登録からクライアントの接続までは [使い方](docs/usage.md) にある。
